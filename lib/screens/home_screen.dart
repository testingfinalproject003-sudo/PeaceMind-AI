...
class _HomeScreenState extends State<HomeScreen>
  ...
  // ===========================================================================
  // ANIMATIONS / AUDIO / NOTIFICATIONS
  // ===========================================================================
  ...
  // ===========================================================================
  // RESTORE TODAY'S EXERCISE COMPLETIONS
  // ===========================================================================
  ...
  bool _isExerciseTask(String id) {
    return id.startsWith('ex_');
  }
  ...
  Routine? _getNextRoutine(List<Routine> routines) {
    ...
    final sorted = [...routines]
      ..sort(
        (a, b) => parseTime(a.time).compareTo(
          parseTime(b.time),
        ),
      );
  ...
  bool _areAllTodayTasksDone(
    RoutineProvider provider,
  ) {
    final todayRoutines =
        provider.getTodayRoutines();

    final userDone = todayRoutines
    ...
    final total =
        ...
        _exerciseTasks.length;

    return total > 0 &&
        (userDone + exerciseDone) == total;
  }
  ...
  _TaskMeta _metaForCategory(
    ...
    switch (category) {
      ...
      case 'afternoon':
        return _TaskMeta(
          subtitle:
              'Slow down and take four calm breaths.',
  ...
  List<PendingTask> _buildPendingTasks(
    RoutineProvider provider,
  ) {
    final now = DateTime.now();
    ...
    for (final h in provider.history) {
      if (lastCompletion[h.routineId] ==
              null ||
          h.completedAt.isAfter(
            lastCompletion[h.routineId]!,
          )) {
        lastCompletion[h.routineId] =
            h.completedAt;
      }
    ...
    for (final r in _exerciseTasks) {
      if (!_completedExerciseIds.contains(
        r.id,
      )) {
        pending.add(
          PendingTask(
  ...
  Future<void> _scheduleRoutineNotifications() async {
    ...
    for (final routine in provider.routines) {
      await _scheduleRoutineNotification(
        routine,
      );
    }
  }

  Future<void> _scheduleRoutineNotification(
    Routine routine,
  ) async {
    ...
    try {
      await _notifications.zonedSchedule(
        ...
        title: 'It is time 🌿',
        body:
            '${routine.title} is ready. Take a moment for yourself.',
  ...
  // Yappy
  //      ↓
  // User clicks Grow My Tree
  //      ↓
  // GardenWidget.growNextTree()
  // ===========================================================================
  ...
  Future<void> _showGardenCelebration({
    ...
    // Yappy plays when celebration card opens.
    await _playYappy();
    ...
    // User dismissed the celebration card.
    // Now start the tree growing animation.
    await _gardenKey.currentState
        ?.growNextTree();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final activeExercises =
        _exerciseTasks
            .where(
              (r) => !_completedExerciseIds
                  .contains(r.id),
            )
            .toList();

    // -------------------------------------------------------------------------
    // UNCOMPLETED USER ROUTINES
    // -------------------------------------------------------------------------
            ...
            Expanded(
              child: CustomScrollView(
                ...
                slivers: [
                  ...
                  if (allUncompleted.isEmpty &&
                      allRoutines.isEmpty)
                    SliverToBoxAdapter(
                      child:
                          _buildSetRoutineCard(),
                    ),
